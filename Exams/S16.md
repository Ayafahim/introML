...
```python
def classification_error(unoccupied, occupied):  
    # Calculate classification error for a group of observations  
    total = unoccupied + occupied  
    return 1.0 - max(unoccupied, occupied) / total if total > 0 else 0  
  
# Total counts of unoccupied and occupied before any splits  
total_unoccupied = 129  
total_occupied = 169  
parent_error = classification_error(total_unoccupied, total_occupied)  
  
# Define the splits from your data with their respective counts  
splits = {  
    'split1': [(45, 1), (47, 66), (8, 33)],  
    'split2': [(76, 20), (16, 47), (8, 33)],  
    'split3': [(25, 0), (55, 23), (20, 77)]  
}  
  
def calculate_purity_gain(split_counts):  
    weighted_error = sum(classification_error(no, yes) * (no + yes) for no, yes in split_counts) / (total_unoccupied + total_occupied)  
    return parent_error - weighted_error  
  
# Calculate purity gains for each split  
purity_gains = {split: calculate_purity_gain(groups) for split, groups in splits.items()}  
  
# Find the best split based on the highest purity gain  
best_split = max(purity_gains, key=purity_gains.get)  
best_purity_gain = purity_gains[best_split]  
  
# Print the best split and its purity gain  
print(f"Hunt's algorithm will select {best_split} with a purity gain of {best_purity_gain:.4f}")  
  
# Print comparison of all splits  
for split, gain in purity_gains.items():  
    print(f"Purity gain for {split}: {gain:.4f}")  
  
# Determine the comparison between splits  
comparisons = []  
for split in splits:  
    if split != best_split:  
        if purity_gains[split] < best_purity_gain:  
            comparisons.append(f"{best_split} is selected over {split} because {best_purity_gain:.4f} > {purity_gains[split]:.4f}")  
        else:  
            comparisons.append(f"{split} is selected over {best_split} because {purity_gains[split]:.4f} > {best_purity_gain:.4f}")  
